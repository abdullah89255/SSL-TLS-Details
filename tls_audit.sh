#!/usr/bin/env bash
# tls_audit.sh — Defensive TLS/SSL auditing script
# Usage: ./tls_audit.sh domain[:port] [domain2[:port] ...]
# Example: ./tls_audit.sh example.com example.net:8443 mail.example.com:25
#
...
# Author: ChatGPT (with emojis) — defensive checks only.
...
OPTIONAL_TOOLS=(testssl.sh sslyze sslscan masscan)
...
for t in "${REQ_TOOLS[@]}"; do
  if command -v "$t" >/dev/null 2>&1; then
    ...
    # Do not exit — will try best-effort but many checks require openssl/curl/nmap
  fi
done
...
probe_with_openssl() {
  ...
  for flag in "-ssl2" "-ssl3" "-tls1" "-tls1_1" "-tls1_2" "-tls1_3"; do
    # Some openssl builds don't accept all flags; attempt and capture output
    {
      printf '\n=== Attempt %s ===\n' "$flag"
  ...
  # Extract cert chain and validity
  {
...
run_testssl() {
  ...
  if command -v testssl.sh >/dev/null 2>&1 || [[ -x "./testssl.sh" ]]; then
    local exe="$(command -v testssl.sh 2>/dev/null || echo "./testssl.sh")"
    log "🧪 Running testssl.sh for $hostport (can be slow) — result -> testssl_${hostport//[:\/]/_}.txt"
    # allow testssl.sh to run fast mode
    "$exe" --fast "$hostport" | tee "${OUTDIR}/testssl_${hostport//[:\/]/_}.txt" || true
    sep
  else
    log "  ⚠️ testssl.sh not available — skipping (optional but recommended)."
  fi
}
...
run_sslyze() {
  ...
  if command -v sslyze >/dev/null 2>&1; then
    log "🐍 Running sslyze (regular) for $hostport -> sslyze_${hostport//[:\/]/_}.txt"
    sslyze --regular "$hostport" 2>&1 | tee "${OUTDIR}/sslyze_${hostport//[:\/]/_}.txt" || true
    sep
  else
    log "  ⚠️ sslyze not installed — skipping (optional)."
  fi
}
...
check_dhparams() {
  ...
  # Attempt to retrieve ServerKeyExchange and extract DH param length via nmap script or testssl output (fallback)
  # We'll try openssl (s_client) and parse for "Server Temp Key" or "DH"
...
check_hsts_and_mixed() {
  ...
  if command -v curl >/dev/null 2>&1; then
    ...
    # Mixed content: look for http:// in body (naive)
    if [[ -s "${bodyfile}" ]]; then
...
for target in "$@"; do
  ...
  run_testssl "$hostport"
  run_sslyze "$hostport"
  check_dhparams "$hostport"
  check_hsts_and_mixed "$host" "$port"

  # If single arg looks like MX or mail port, also run STARTTLS probe
  if [[ "$port" -eq 25 || "$port" -eq 587 || "$port" -eq 465 ]]; then
    check_starttls_smtp "$host" "$port"
  else
    # Try resolving MX if the argument is a domain with no mail host passed
    if [[ "$port" -eq 443 ]]; then
      # check MX records and probe their STARTTLS (first MX only)
...
🚨 IMPORTANT:
 ...
 - It attempts many non-invasive probes (openssl/nmap/testssl/sslyze). Some organizations may still detect scanning as suspicious.
